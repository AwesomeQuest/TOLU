...
# ╔═╡ 4560c690-acab-11ef-053c-e906fa9237dd
begin
...
# ╔═╡ 7c977345-4c79-4649-9d1a-ea2032e2270e
begin
	T = (2:10) .*10 .+ 273.15
	μ = [1.003, 0.799, 0.657, 0.548, 0.467, 0.405, 0.355, 0.316, 0.283]*1e-3
...
# ╔═╡ 31e9ac19-454f-4461-8b4e-7458f81ce307
β1, β1e = let
	A = [ones(size(μ))  T .^-1  T.^-2]
	
	β = A\log.(μ)
	β, norm(A*β - log.(μ))
end

# ╔═╡ 0fa7e190-22e3-4981-aba2-a4caba5fc790
function newton(f,x0,tol::AbstractFloat)
	...
	while norm(curr - prev) > tol
		prev, curr = curr, curr - Df(curr)\f(curr)
	end

	curr
end

# ╔═╡ c00b5f5a-0049-4fb5-baa1-4b7871cd21f7
function newton(f,x0,N::Integer = 10)
	Df(x) = ForwardDiff.jacobian(f,x)
...
# ╔═╡ f6c02a58-6075-4e97-87d4-9ed3878fd1ce
μ12(T,c1,c2,c3) = exp(c1 + c2/T + c3/T^2)

# ╔═╡ 71d71531-58bf-4516-acb0-6df6a3a19c39
β2, β2e = let
	μ̂(T,c1,c2,c3) = exp(c1 + c2/T + c3/T^2)
	F(c) = μ̂.(T,c...) - μ
	output = newton(F,ones(BigFloat,3), big"1e-5")
	output, norm(F(output))
end

# ╔═╡ 74e01fe9-7027-44a3-9553-9e654f4ae1ad
μ3(T,A,B,C) = exp(A/T + B*T + C*T^2)

# ╔═╡ 22f1ffdf-4075-4f97-8475-2993d18a9b40
β3, β3e = let
	μ̂(T,A,B,C) = exp(A/T + B*T + C*T^2)
	F(c) = μ̂.(T,c...) - μ
	DF(c) = ForwardDiff.jacobian(F,c)
	output = newton(F,zeros(BigFloat,3), big"1e-5")
	output, norm(F(output))
end
...
function simpson(f,a,b,n)
	...
	h/6*(
		f(a) + 
		2sum(f(a+i*h) for i in 1:n-1; init = 0) + 
		4sum(f(a+h/2+i*h) for i in 0:n-1; init = 0) + 
		f(b)
	)
end

# ╔═╡ 0bc22b73-332c-498c-9f6b-05b2c77f2735
simpson(x->1/x, 1,2,10)

# ╔═╡ c645bdf2-e171-4543-97a9-09fbf5007e24
function adapquad(f,a,b,tol)
	I0 = simpson(f,a,b,1)
	I1 = simpson(f,a,b,2)
...
V(y,n=0.5e-10; T0 = T0, T1 = T1) = begin
	
	T(x) = T0 + x*(T1-T0)/L
	mu(ξ) = μ12(T(ξ), β1...)
	K = V0 / adapquad(ξ -> 1/mu(ξ), 0,L,n)[1]
	K * adapquad(ξ-> 1/mu(ξ), 0,y,n)[1]
end
...
# ╔═╡ b70792f0-a5ff-4c36-8e0c-eaefd71fd2b4
let
	tend = 10
	fps = 30
	xs = range(0,L, 70) |> collect
	vs = V.(xs; T0=T1, T1=T0)
...
# ╔═╡ b5bbc911-50ce-4e3a-bb1f-4fa04beb8516
let
	tend = 10
	fps = 30
	xs = range(0,L, 70) |> collect
...
# ╔═╡ 9bda3740-51dc-4952-9cc1-5e9f1d1b2873
let
	tend = 10
	fps = 30
	xs = range(0,L, 70) |> collect
...
manifest_format = "2.0"
project_hash = "fa311bc0db712a31ce75c011fca126befb13c2c5"
