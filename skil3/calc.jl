...
function yf(y0, h, N, f, step)
	iterated((x->(x[1] + step(f, x[1] ,h, x[2]*h),x[2]+1)), (y0,0)) |> x->Iter.take(x,N+1) .|> first
end
...
# ╔═╡ 9be76605-ec8b-457b-bac2-83a2433b4fdb
function RKstep(f,y,h,t)
	k1 = f(t,y)
	k2 = f(t+h/2, y+h/2*k1)
	k3 = f(t+h/2, y+h/2*k2)
	k4 = h*f(t+h, y+h*k3)
...
let
	...
	fig,ax = lines(t0:h:tend, output .|> x->x[1], label=L"y_1 \text{ með } y_{0_1} = %$(y0[1])")
	lines!(t0:h:tend, output .|> x->x[2], label=L"y_2 \text{ með } y_{0_2} = %$(y0[2])")
	output2 = yf(y1, h, N, y, eulerstep)
	lines!(t0:h:tend, output2 .|> x->x[1], label=L"y_1 \text{ með } y_{1_1} = %$(y1[1])")
	lines!(t0:h:tend, output2 .|> x->x[2], label=L"y_1 \text{ með } y_{1_2} = %$(y1[2])")
	axislegend(ax)
	ax.xlabel = L"y_1"
	ax.ylabel = L"y_2"
	current_figure()
...
# ╔═╡ d47d3152-73d2-49a0-b058-90ee7f58e4b3
let
	
	y0 = [40,9]
	t0 = 0; tend=35; N = 600; h = tend/N;
	output = yf(y0, h, N, y, eulerstep)

	t0 = 0; tend=35; N = 300; h = tend/N;
	rkoutput = yf(y0, h, N, y, RKstep)
	...
	lines!(ax, output .|> x->Point2f(x...))
	lines!(ax, rkoutput .|> x->Point2f(x...))
	#lines!(ax, sol.u .|> x->Point2f(x...))

	fig
end

# ╔═╡ 09b427c5-b7d1-4935-9fdf-ecd2c6afe47c
let
	y0 = [40,9]
	t0 = 0; tend=50; N = 100*2 .^(0:13); h = tend ./N;

	rkN = 60000; rkh = tend/rkN
	rkts = rkh |> x->range.(t0,tend; step=x)
	rkoutput = yf(y0,rkh,rkN,y,RKstep)
	sol = linear_interpolation(rkts, rkoutput)
	
	ts = h .|> x->range.(t0,tend; step=x)
	output = yf.(Ref(y0), h, N, y, eulerstep)
	
	# RMSEs = zeros(length(output))
	RMSEs = fill((0.,0.),length(output))
	enderr = fill([0.0,0.0],length(output))
	for (ind,out) in enumerate(output)
		err = out - sol.(ts[ind])
		enderr[ind] = out[end] - sol(50) .|> abs
		RMSEs[ind] = √(sum(x->x[1]^2, err)/length(err)) , √(sum(x->x[2]^2, err)/length(err))
	end
	# scatter(
	# 	N, RMSEs .|> x->x[1],
	# 	axis=(
	...
	# 	)
	# )
	# scatter!(N, RMSEs .|> x->x[2])
	# current_figure()
	second(x) = x[2]
	1 .- second.(enderr[1:end-1]) ./ second.(enderr[2:end]) .|> abs .|> log2 |> diff
	1 .- first.(enderr[1:end-1]) ./ first.(enderr[2:end])   .|> abs .|> log2 |> diff
	1 .- first.(RMSEs[1:end-1]) ./ first.(RMSEs[2:end])     .|> abs .|> log2 |> diff
	1 .- second.(RMSEs[1:end-1]) ./ second.(RMSEs[2:end])   .|> abs .|> log2 |> diff
end

# ╔═╡ 6904606b-6341-4e6a-90bd-225dda91ae86
let
	y0 = [40,9]
	t0 = 0; tend=50; N = 100*2 .^(0:12); h = tend ./N;

	rkN = 100*2^14; rkh = tend/rkN
	rkts = rkh |> x->range.(t0,tend; step=x)
	rkoutput = yf(y0,rkh,rkN,y,RKstep)
	sol = linear_interpolation(rkts, rkoutput)

	prob = ODEProblem((x,p,t)->y(t,x), y0, (t0,tend))
	Tsol = solve(prob, Tsit5(), reltol = 1e-10, abstol = 1e-10)
	
	ts = h .|> x->range.(t0,tend; step=x)
	output = yf.(Ref(y0), h, N, y, RKstep)
	
	#RMSEs = fill((0.,0.),length(output))
	RMSEs = zeros(length(output))
	TRMSEs = zeros(length(output))
	#enderr = fill([0.0,0.0],length(output))
	enderr = zeros(length(output))
	Tenderr = zeros(length(output))
	for (ind,out) in enumerate(output)
		err = out - sol.(ts[ind])
		Terr = out - Tsol.(ts[ind])
		enderr[ind] = out[end] - sol(tend) |> norm
		Tenderr[ind] = out[end] - Tsol(tend) |> norm
		#RMSEs[ind] = √(sum(x->x[1]^2, err)/length(err)) , √(sum(x->x[2]^2, err)/length(err))
		#RMSEs[ind] = √(sum(x->x[1]^2, err)/length(err)) + √(sum(x->x[2]^2, err)/length(err))
		RMSEs[ind] = √(sum(norm, err)/length(err)) + √(sum(norm, err)/length(err))
		TRMSEs[ind] = √(sum(norm, Terr)/length(Terr)) + √(sum(norm, Terr)/length(Terr))
	end
	second(x) = x[2]
	#first.(RMSEs[1:end-1]) ./ first.(RMSEs[2:end])     .|> log2 |> diff
	#second.(RMSEs[1:end-1]) ./ second.(RMSEs[2:end])   .|> log2 |> diff
	RMSEs[1:end-1] ./ RMSEs[2:end]
	#first.(enderr[1:end-1]) ./ first.(enderr[2:end])   
	#second.(enderr[1:end-1]) ./ second.(enderr[2:end]) 
	enderr[1:end-1] ./ enderr[2:end]
	RMSEs
	TRMSEs
	Tenderr[1:end-1] ./ Tenderr[2:end]
end
...
manifest_format = "2.0"
project_hash = "bb8ca50592e50b3a8c67adfb5d36579866c73a06"
