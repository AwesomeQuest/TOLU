...
# ╔═╡ c9453880-b21c-11ef-0c9b-eb5115e3a370
begin
	using Makie, CairoMakie
	using LinearAlgebra, IterTools, FlexiMaps
...
# ╔═╡ f22fc0d3-b179-4361-9fb7-38b84f537415
TableOfContents()

# ╔═╡ 27596c7f-0793-4e23-880a-dcc081acaa15
md"""
## Functions
"""

# ╔═╡ 2daa48b1-0365-41ee-954b-d8759bb9da02
function divergence(f,x) 
	tr(ForwardDiff.jacobian(f,x))
end

# ╔═╡ 2428f2bb-13cd-4548-8dab-a4f3ea305ef1
function curl(f,x)
...
# ╔═╡ 1f46df5d-b043-4686-8371-f780ff1fbede
function laplacian(f,x)
	ForwardDiff.gradient(z->divergence(f,z),x) - curl(z->curl(f,z),x)
end

# ╔═╡ d99fd6f0-36be-4264-b9b6-858cf2a390d2
begin
	second(x) = x[2]
	r1,r2,k,d,j,w = 1,0.1,7,1,1,0.3
	y(x,t) = [
		r1*x[1]*(1-x[1]/k) - w*x[1]/(d+x[1])*x[2]
		r2*x[2]*(1-j*x[2]/x[1])
	] |> Point2
	Dy(y1,y2) = [
		-d*y2*w/(d+y1)^2+r1-2*r1*y1/k -y1*w/(d + y1)
		j*r2*y2^2/y1^2 r2*(-2j*y2+y1)/y1
	]
	w2 = 1
	y2(x,t) = [
		r1*x[1]*(1-x[1]/k) - w2*x[1]/(d+x[1])*x[2]
		r2*x[2]*(1-j*x[2]/x[1])
	] |> Point2
	Dy2(y1,y2) = [
		-d*y2*w2/(d+y1)^2+r1-2*r1*y1/k -y1*w2/(d + y1)
		j*r2*y2^2/y1^2 r2*(-2j*y2+y1)/y1
	]
end

# ╔═╡ 1e3b3a12-b329-47a0-a20a-c860e5c72c28
(x->laplacian(z->[y2(z,0);0],[x;0]))([1,1])

# ╔═╡ 3ef83d54-5125-45a9-9e1c-4cf8946f133e
divergence(x->y(x,0), [1,1])

# ╔═╡ d10f473c-96b3-441d-963a-516ef408eefd
begin
	function integ(f,u0,tspan,N, step = rkstep)
		t0, T = tspan
		h = T/N
		data = iterated(
			(x->(
				x[1] + step(f, h, x[1], x[2]),
				x[2] + h)
			), (u0,t0)
		) |> x->Iter.take(x, N+1)
		linear_interpolation(data .|> x->x[2],data .|> x->x[1], extrapolation_bc=Line())
	end
	const eulerstep(f,h,x,t) = h*f(x,t)
	function rkstep(f,h,x,t)
		k1 = f(x         , t      )
		k2 = f(x + h*k1/2, t + h/2)
		k3 = f(x + h*k2/2, t + h/2)
		k4 = f(x + h*k3  , t + h  )
	...
	function funcRMSE(f1,f2,ts)
		√sum(x->norm(x)^2/length(ts), f1(t)-f2(t) for t in ts)
	end
end

# ╔═╡ 57fe77c6-92b9-4985-ad8b-e0175a1655a8
let
	y0 = Point2([2,2])
	t0,tend = tspan = (0,10)
	rkout = integ(y, y0,tspan,100, rkstep)
	euout = integ(y, y0,tspan,100, eulerstep)
	lines( rkout.(t0:0.01:tend), label="Runge-Kutta")
	lines!(euout.(t0:0.01:tend), label="Euler")
	axislegend(position=:rb)
	current_figure()
end

# ╔═╡ df226d94-b0f9-4e07-afea-2e141d5bf102
md"""
## The vecotor field
"""

# ╔═╡ 2162c571-786a-4bc1-8d10-3d00854ae7db
let
	y01 = Point2([2,2])
	y02 = Point2([4,4])
	y03 = Point2([10,10])
	t0,tend = tspan = (0,300)
	rkout = integ(y2, y01,tspan,10000, rkstep)
	rkout2 = integ(y2, y02,tspan,10000, rkstep)
	rkout3 = integ(y2, y03,tspan,10000, rkstep)
	...
	lines!( rkout.(t0:0.01:tend);  )
	lines!( rkout2.(t0:0.01:tend); )
	lines!( rkout3.(t0:0.01:tend); )
	fig
end

# ╔═╡ 3fdaedef-7360-4bd7-a1d1-5ee0ed5e3982
md"""
## The eigen values of the jacobian
"""

# ╔═╡ dd3dec89-0872-457e-8fea-288cd61ddf40
let
	y01 = Point2([2,2])
	y02 = Point2([4,4])
	y03 = Point2([10,10])
	t0,tend = tspan = (0,300)
	rkout = integ(y2, y01,tspan,1000, rkstep)
	rkout2 = integ(y2, y02,tspan,1000, rkstep)
	rkout3 = integ(y2, y03,tspan,1000, rkstep)

	step = 0.1:0.18:10

	pts = [Point2(x,y) for x in step for y in step]
	pts2 = [Point2(x+0.1,y+0.1) for x in step for y in step]
	colors = [
		ForwardDiff.jacobian(x->y2(x,0),p) |> Matrix |> eigvals |> first |> real |> sign
		for p in pts
	] 
	colors2 = [
		ForwardDiff.jacobian(x->y2(x,0),p) |> Matrix |> eigvals |> second |> real |> sign
		for p in pts
	] 
	fig,ax,l = scatter(pts, color= colors)
	Colorbar(fig[1,2],l)
	l = scatter!(pts2, color= colors2)
	Colorbar(fig[1,3],l)
	lines!( rkout.(t0:0.01:tend);  )
	lines!( rkout2.(t0:0.01:tend); )
	lines!( rkout3.(t0:0.01:tend); )
...
# ╔═╡ 213e79a7-0b6c-412b-a006-cbd05fd6fcc6
rand(2,2) |> eigvals

# ╔═╡ 6840364c-76b6-4c60-a303-b934f42595d7
md"""
## Eigen Vectors of the jacobian
"""

# ╔═╡ ad4154b9-050d-439c-85f1-f963e12db852
let
	y01 = Point2([2,2])
	y02 = Point2([4,4])
	y03 = Point2([10,10])
	...
	ys = range(0.1,10, N)
	ps = [Point2(x,y) for x in xs for y in ys]
	ns = [ForwardDiff.jacobian(x->y2(x,0),p) |> Matrix |> eigvecs |> second .|> real |> Vec2 for p in ps]
	ls = norm.(ns)
	fig,ax,l = arrows(
		ps, ns, 
		normalize = true,
...
# ╔═╡ 8bafb4ee-accb-4cbe-b07b-d4318a889189
md"""
## The divergence
"""

# ╔═╡ 3d042f30-d70f-4b86-8cef-057b5d5e775b
let
	y01 = Point2([2,2])
	y02 = Point2([4,4])
	y03 = Point2([10,10])
	...
	rkout = integ(y2, y01,tspan,1000, rkstep)
	rkout2 = integ(y2, y02,tspan,1000, rkstep)
	rkout3 = integ(y2, y03,tspan,2000, rkstep)

	step = 0.01:0.18:10

	pts = [Point2(x,y) for x in step for y in step]
	colors = [divergence(x->y2(x,0),p) |> sign for p in pts]

	fig,ax,l = scatter(pts, color= colors)
	Colorbar(fig[1,2],l, 
		ticks = [maprange(sinh, extrema(colors)..., 5) ; 0],
		tickformat = vals->["$(round(Int,sinh(val)))" for val in vals],
	)
	lines!( rkout.(t0:0.01:tend);  )
	lines!( rkout2.(t0:0.01:tend); )
	lines!( rkout3.(t0:0.01:tend); )
...
# ╔═╡ 8a017402-c5c8-409b-a908-6882dd7c8f24
md"""
## The Curl
"""

# ╔═╡ 076c1626-4f90-471c-b1f9-887eb3ee861e
let
	y01 = Point2([2,2])
	y02 = Point2([4,4])
	y03 = Point2([10,10])
	...
	pts = [Point2(x,y) for x in step for y in step]
	colors = [curl(x->[y2(x,0);0],[p;0])[3] |> asinh for p in pts]

	fig,ax,l = scatter(pts, color= colors)
	Colorbar(fig[1,2],l, 
		#ticks = [maprange(sinh, extrema(colors)..., 100) ; 0],
		#tickformat = vals->["$(round(Int,sinh(val)))" for val in vals],
	)
	lines!( rkout.(t0:0.01:tend);  )
	lines!( rkout2.(t0:0.01:tend); )
...
# ╔═╡ 0d0ee7b1-4b1f-4ca5-8d21-18aa4cc96261
md"""
## The Laplacian
"""

# ╔═╡ 3afd233c-34a8-49f3-bb8f-f578db6e34c6
let
	y01 = Point2([2,2])
	y02 = Point2([4,4])
	y03 = Point2([10,10])
	...
	ys = range(0.,10, N)
	ps = [Point2(x,y) for x in xs for y in ys]
	ns = map(x->laplacian(z->[y2(z,0);0],[x;0])[1:2] |> k->Vec2(k...), ps) 
	ls = norm.(ns)
	fig,ax,l = arrows(
		ps, ns, 
		normalize = true,
...
# ╔═╡ 835854f3-7920-4d58-92cc-ef2ee796b6f6
md"""
## The divergence of the laplacian
"""

# ╔═╡ eadb6c7d-60fe-434d-86b8-f8f50e8b81fd
let
	y01 = Point2([2,2])
	y02 = Point2([4,4])
	y03 = Point2([10,10])
	...
	pts = [Point2(x,y) for x in step for y in step]
	colors = [divergence(x->laplacian(z->[y2(z,0);0],[x;0])[1:2],p) |> asinh for p in pts]
...
manifest_format = "2.0"
project_hash = "1c648b4617f452da81114ecf71b52cf09d9a48a0"
