...
# ╔═╡ 3807f620-b184-11ef-2e7c-a90b165a6c37
begin
...
# ╔═╡ 86fe08f1-3a51-4c2e-9f43-7cde3cdcc53e
TableOfContents()

# ╔═╡ 9828a424-7f23-4d66-9a47-a82236275806
md"""
# Uppsetning
"""

# ╔═╡ e9acc03d-6cfa-44ca-80cf-9d29c4024721
α, β, γ, δ = 0.5, 0.01, 0.005, 0.2

# ╔═╡ 131fc581-9745-4e06-9e91-0f4d0c6987bd
y(x,t) = [
	α*x[1]      - β*x[1]*x[2]
	γ*x[1]*x[2] - δ*x[2]
]

# ╔═╡ 5d1985b2-e1a5-4666-810b-48b3d67139d7
const eulerstep(f,h,x,t) = h*f(x,t)

# ╔═╡ a1c25a78-085d-4491-86a2-59523287c33c
...
# ╔═╡ 48aef66b-ace3-4463-a991-134c9c365eb1
...
# ╔═╡ 28ce28c9-2cc6-4291-a45b-ba25df633640
function funcRMSE(f1,f2,ts)
	√sum(
		x->norm(x)^2/length(ts), 
		f1(t)-f2(t) for t in ts
	)
end

# ╔═╡ e109efbe-7717-4a34-abb4-81e3a8f3dfd6
md"""
# Útreikningar
"""

# ╔═╡ fd51f148-dece-4757-a4ab-818f34b8c7b8
md"""
## Dæmi 3
"""

# ╔═╡ 0e9c816d-30b5-4707-b06f-4416455f0dd0
let
	t0 = 0; tend=35; N = 600;
	y0 = [40.,9.]
	output = integ(y,y0,(t0,tend), N)
	y02 = [50.,40]
	output2 = integ(y,y02,(t0,tend), N)
	ts = t0:0.1:tend
	...
	lines!(ax, output.(ts) .|> x->Point2f(x...))
	lines!(ax, output2.(ts) .|> x->Point2f(x...))

	fig
end

# ╔═╡ 02270ec5-afd7-4541-87ef-8e206b4a03fc
md"""
## Dæmi 4
"""

# ╔═╡ 8813d433-2efc-4054-aca0-64a6495def7f
md"""
### RMSE aðferð
"""

# ╔═╡ 7d897c04-ffbf-4627-92bb-4de9c87b8cbb
100 * 2 .^(0:10)

# ╔═╡ 61aff02c-5340-4003-a4e8-674bb138778d
let
	y0 = [40,9]
	t0,tend = tspan = (0,10)
	
	RMSEs = funcRMSE.(
		[integ(y, y0,tspan,100, rkstep)], 
		integ.(y, [y0],[tspan],100 * 2 .^(0:10), eulerstep), 
		[t0:0.1:tend]
	)

	RMSEs[2:end] .\ RMSEs[1:end-1]
end

# ╔═╡ 151cefb1-996d-406e-bf18-a69df7b5719f
md"""
### Endagildis aðferð
"""

# ╔═╡ 1300b6e0-6b27-4ba7-b337-59eb806b3511
let
	y0 = [40,9]
	t0,tend = tspan = (0,30)
	trueval = integ(y, y0,tspan,1000, rkstep)
	stps = integ.(y, [y0],[tspan],100 * 2 .^(0:13), eulerstep)
	errs = [st[end] - trueval[end] for st in stps]  .|> norm

	errs[2:end] ./ errs[1:end-1]
end

# ╔═╡ 9e588e46-8505-489d-932d-36950465e47b
md"""
## Dæmi 6
"""

# ╔═╡ d615d484-f68d-4313-b295-098b539b954e
let
	y0 = [40,9]
	t0,tend = tspan = (0,50)
	
	RMSEs = funcRMSE.(
		[integ(y, y0,tspan,100000)], 
		integ.(y, [y0],[tspan],2 .^(6:18)), 
		[t0:0.0001:tend]
	)
...
# ╔═╡ 3a3e1c22-b17c-4c98-9a1c-01b58460284f
let
	y0 = [40,9]
	t0,tend = tspan = (0,30)
	@time "true" trueval = integ(y, y0,tspan,10000)
	@time "steps" stps = integ.(y, [y0],[tspan],100 * 2 .^(0:11))
	errs = [st[end] - trueval[end] for st in stps]  .|> norm
