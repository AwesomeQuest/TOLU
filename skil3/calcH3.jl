...
# ╔═╡ ed3fafc0-b24b-11ef-13ec-d90950a62afd
begin
...
# ╔═╡ 86af9236-490f-40bf-bedd-f0b7ea6dc3fa
begin
	a1,b1,a2,b2,d1,d2 = 5,3,0.1,2,0.4,0.01
	F(x,t, b1=b1) = [
		x[1] * (1 - x[1]) - a1 * x[1] * x[2] / (1 + b1 * x[1])
		
		a1 * x[1] * x[2] / (1 + b1 * x[1]) - d1 * x[2] - a2 * x[2] * x[3] / (1 + b2)
		
		a2 * x[2] * x[3] / (1 + b2 * x[2]) - d2 * x[3]
	]
end

# ╔═╡ c75737e7-9c7c-42ce-8414-310945e07190
md"""
...
# ╔═╡ 88116c63-264e-4660-896a-f997e94d0c62
F([1,2,3],0)

# ╔═╡ e62ce3f6-ea6f-4745-9ae3-63f69902f014
begin
...
# ╔═╡ 7fbfbf28-412e-467e-aa02-7629afceaa00
function funcseries(t0,tend, f; labels=[])
	xs = t0:(tend-t0)/500:tend
	CairoMakie.series(xs, f.(xs) |> x->mapreduce(permutedims, vcat,x)')
end

# ╔═╡ 4ed1b9d4-3512-4aa5-874a-9a5f4514495f
let
	t0,tend = tspan = (0,400)
	x0 = [0.8,0.1,8]
	out = integ((x,t)->F(x,t,3),x0,tspan, 40000)
	funcseries(tspan...,out; labels=[L"X",L"Y",L"Z"])
	axislegend()
	current_figure()
end

# ╔═╡ 060d0ced-1c8f-4b5a-b8c8-df1e756f36f8
@bind b1t PlutoUI.Slider(range(0,10,50))

# ╔═╡ 18ff7118-f675-46ce-ae82-1d0988f76549
let
	bs = range(0,10,600)
	b = Observable(bs[1])
	t0,tend = tspan = (0,400)
	x0 = [0.8,0.1,8]
	out = @lift integ((x,t)->F(x,t,$b),x0,tspan, 40000)
	ts = range(t0,tend, 2000)
	fig=Figure(); 

	xs = [@lift($out.(ts) .|> x->x[i]) for i in 1:3]
	axs = []
	scales = [(1,1.5), (1,1), (1,2), ]
	for (x,color, side,sc) in zip(
			xs, 
			[:blue,:red,:green], 
			[:left,:left,:right], 
			scales)
		ax = Axis(
			...
			xticklabelsvisible = side == :right ? true : false
		)
		push!(axs,ax)
		lines!(ax, ts, x, color = color)
		ax.limits = (tspan,extrema(x[]) .* sc)
		on(x) do i
			ax.limits = (tspan,extrema(i) .* sc)
		end
	end

	record(fig, "video.mp4", bs) do i
		b[] = i
		axs[3].title = L"b_1 = %$(round(i, digits=4))"
	end |> x-> md"$(LocalResource(x, :loop=>true))"
end

# ╔═╡ 6743a100-7ddc-46ce-9611-0b0a6e2ae80b
@bind az PlutoUI.Slider(range(0,2pi,100))

# ╔═╡ b88dd3e1-cbf7-4a77-bc68-0fc659da1d20
@bind el PlutoUI.Slider(range(0,pi/8,100))

# ╔═╡ bbef3251-6494-4dce-b46c-07db5c257b85
let
	t0,tend = tspan = (0,400)
	x0 = [0.8,0.1,8]
	out = integ((x,t)->F(x,t,3),x0,tspan, 4000)
	ts = range(t0,tend,2000)
	xs = [out.(ts) .|> x->x[i] for i in 1:3]
	
	fig = Figure(); ax = Axis3(fig[1,1],azimuth = az, elevation=el)
	lines!(ax,xs...,)
	fig
end

# ╔═╡ e1be5fc9-d444-4905-b7a8-fd5a3ab8d536
md"""
## Competitive Lotka-Volterra
"""

# ╔═╡ 3be2bcdc-7c0f-4426-a517-3d6901c8d83d
begin
	N = 5
	α = rand(N,N)
	r = rand(N)
	dx(x) = [r[i]*x[i]*(1-α[i,:]'x) for i in eachindex(x)]
	x0 = rand(N)
end

# ╔═╡ a7fa44a8-7f39-41bb-8e6a-222b8e1068c5
let
	t0,tend = tspan = (0,50)
	out = integ((x,t)->dx(x), x0, tspan, 1000)
	out(5)
	funcseries(t0,tend,out)
	axislegend()
	current_figure()
end
...
manifest_format = "2.0"
project_hash = "07cad4f21a3380a9b0466982a52e9a53de88d92c"
