...
# ╔═╡ 2504a7fd-2778-4b95-96a1-c1d2190f4ad6
begin
	using CairoMakie, MakieExtra, PlutoUI
	using ForwardDiff, LinearAlgebra, Peaks
	using Unitful, Measurements
	using Typstry, Printf
end

# ╔═╡ 58e56f3f-22d3-4f3f-b1e6-cc7643885b6f
using Unitful: mm, Pa, g, rad, Hz, s

# ╔═╡ 4617aa91-bd9f-4209-b763-448233790aa9
f = x->cos(x)*cosh(x)+1

# ╔═╡ efe6d471-ef05-4236-b339-5958b89de34e
let
	xs = range(0,10,1000)
	lines(xs, f.(xs),
		axis=(
			yscale=AsinhScale(0.1),
			#yticks=LinearTicks(15) |> LogTicks, 
			xticks=LinearTicks(22),
			#ytickformat=vals->["$(round(v,sigdigits=3))" for v in vals],
			yminorticksvisible=true
		)
	)
end

# ╔═╡ 3f49722f-0b1a-4e5b-a441-f5d9197f1bd0
function bisect(f, a, b, xtol, ytol=1)
	sign(f(a)) == sign(f(b)) && throw(error("Bad range"))
    m = (a + b)/2

	while (b-a)/2 > xtol && abs(f(m)) > ytol
...
# ╔═╡ 2b8bd16b-088f-40e1-aff3-87a6408eadf3
bisect(f, 0,2, 1e-5)

# ╔═╡ 5b53b9d7-e6b4-4de7-bd5a-7521d2d575e4
let
	EI = 1.09*1010Pa*mm^2
	λ  = 0.8g/mm^3
	k  = (bisect(f, 0,2, 1e-5) ± 5e-5)/30mm
	k^2*sqrt(EI/λ) |> Hz
end

# ╔═╡ 4aacc24c-db2d-427a-b1bb-8de5bfb04f62
let
	N = 1000
	xs = range(0,20*π,N)
	ys = (x->clamp(f(x), -1,1)).(xs)
	fig,ax = lines(xs[2:end], diff(ys), axis=(yticks=LinearTicks(16),xticks=LinearTicks(27)))
	extrind = [findminima(diff(ys)).indices; findmaxima(diff(ys)).indices]
	scatter!(xs[extrind], diff(ys)[extrind])
	fig
end

# ╔═╡ 0d7ad348-1467-4242-9b48-afec511c2861
let
	N = 100000
	xs = range(big"0",20*π,N)
	ys = (x->clamp(f(x), -1,1)).(xs)
	extrind = [findminima(diff(ys)).indices; findmaxima(diff(ys)).indices] |> sort

	Δ = 0.3
	
	rts = []
	for extr in extrind
		push!(rts, bisect(f,xs[extr-1]-Δ, xs[extr-1]+Δ, 1e-7, 1e-2))
	end
	rts .|> f
end

# ╔═╡ 02fb8ea6-3390-4a47-8372-dc2f75c9464c
function newton(f, x0, tol::AbstractFloat, ftol=1.0)
    ...
    while abs(prev-curr) > tol
		...
		counter += 1
		counter > 100 && throw(error("Newton Failure"))
    end
    curr
end

# ╔═╡ 3216406b-b6a9-40c9-b218-85587e348488
function newton(f, x0, N::Integer)
    df(x) = ForwardDiff.derivative(f,x)
    prev = x0
    curr = x0 - f(x0)/df(x0)
...
# ╔═╡ 246c0a5a-4731-4c42-a604-0541b2363c15
newton(f, 4.8, 1e-5)

# ╔═╡ 790ca260-2e96-404b-aeb4-a1ad2db17e1a
let
	EI = 1.09*1010Pa*mm^2
	λ  = 0.8g/mm^3
	k  = (newton(f, 4.8, 1e-5) ± 5e-5)/30mm
	k^2*sqrt(EI/λ) |> Hz
end

# ╔═╡ 211eeaba-7d1f-4bb5-8936-4ddd56b11c85
rts, fvals,  = let
	N = 1000
	xs = range(big"0",20*π,N)
	ys = (x->clamp(f(x), -1,1)).(xs)
	extrind = [findminima(diff(ys)).indices; findmaxima(diff(ys)).indices] |> sort
	rts = BigFloat[]
	for (ind,extr) in enumerate(extrind)
		push!(rts, newton(f,xs[extr-1], 4))
	end
	rts, f.(rts)
end

# ╔═╡ c4799955-2f42-4c96-9727-0990b50f9019
typst"\(round.(Float64.(rts), digits=3); mode=code)" |> print

# ╔═╡ 83ab1952-6d17-4d44-a4b2-532401e82de2
let
	N = 1000
	xs = range(0,20*π,N)
	ys = (x->clamp(f(x), -1,1)).(xs)
	fig,ax = lines(xs,ys,
		axis=(yticks=LinearTicks(16),xticks=LinearTicks(27))
	)
	scatter!(rts, f)
	fig
end

# ╔═╡ f3f94c69-5c0f-4ba6-93b2-e52ff271519a
let
	tihng = rts .|> ustrip
	for i in tihng
		@printf "\"%06.4f\"," i
...
# ╔═╡ a95e19eb-ecfb-4308-a36d-585c07fe8f07
ω(k,EI,λ) = k^2 * sqrt(EI/λ)

# ╔═╡ f306196c-155f-415d-a1a5-933a52bfdc8f
let
	tihng = ω.(rts./30mm, 1.09*1010Pa*mm^2, 0.8g/mm^3) .|> Hz .|> ustrip
	for i in tihng
		@printf "\"%05.3f\"," i
	end
end

# ╔═╡ 3343cb26-12cf-4bce-a037-21ec82ef311b
begin
	A(
		x, 
		L = 30mm,
		...
		cos(k*x) + 
		(cos(k*L) + cosh(k*L))/(sin(k*L) + sinh(k*L)) *
		(sin(k*x) - sinh(k*x))
	B(
		t;
		L = 30mm,
...
# ╔═╡ 3730cc35-4668-4799-90a8-3de9358a93e1
ω(Float64(rts[1])/30mm, 1.09*1010Pa*mm^2, 0.8g/mm^3) |> upreferred

# ╔═╡ bf83fe7d-531c-4c72-9053-d210a4079699
typst"\(@. round(ω(Float64(rts)/30mm, 1.09*1e10Pa*mm^2, 0.8g/mm^3) |> Hz |> ustrip, sigdigits=6); mode=code)" |> print

# ╔═╡ eba6f79f-015e-4d7e-adf9-9ada633bfbc2
let
	N = 2
	tend = 2π/ω(rts[N]/30mm, 1.09*1e10Pa*mm^2, 0.8g/mm^3)
	frames = 500
	fps = frames/tend |> s^-1
	t = Observable(0.0s)
	ts = range(t[], tend, ceil(Int,tend*fps))
	@show ts
	fig = Figure()
	ax = Axis(fig[1,1],
			limits = ((-1,31), (-4,4)),
	...
	lines!(ax,
		0..30,
		CairoMakie.@lift(x->(A(x*mm, 30mm, rts[N]/30mm)*B($t, k=rts[N]/30mm) |> ustrip))
	)
	record(fig, "video.mp4", ts,framerate = 60) do i
	    t[] = i
	end
end |> x-> md"$(LocalResource(x, :loop=>true))"

# ╔═╡ 86d2c16d-4d16-4a2c-89d5-5264fe7b5c2f
2π/ω(rts[2]/30mm, 1.09*1e10Pa*mm^2, 0.8g/mm^3) |> s

# ╔═╡ d2b6c1bb-41d1-498f-877a-67e9a844d8e8
let
	N = 2
	tend = 2π/ω(rts[N]/30mm, 1.09*1010Pa*mm^2, 0.8g/mm^3)
	fps = 30/s
	t = Observable(0.0s)
	ts = range(t[], tend, ceil(Int,tend*fps))
	fig = Figure()
	...
	lines!(ax,
		0..30,
		x->A(x*mm, 30mm, rts[N]/30mm)*B(4tend/6, k=rts[N]/30mm) |> ustrip
	)
	fig
end
...
manifest_format = "2.0"
project_hash = "c26cdb24f1e5e6a0a945d65c87287f2e32973051"
